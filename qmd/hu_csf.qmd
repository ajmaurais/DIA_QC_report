---
title: "Human CSF plate 1A DIA QC report"
toc: true
format:
    html:
        code-fold: true
...
bar_chart(tic, 'TIC area', dpi=plot_dpi)
```
...
std_proteins = ['PRTC', 'sp|P00924|ENO1_YEAST']

for std in std_proteins:
    peptide_rt_plot(std, conn, dpi=plot_dpi)
```
...
bar_chart(agg, 'Number of precursors', legend_title='Missed cleavages', dpi=plot_dpi)
```
...
bar_chart(agg, 'Number of precursors', legend_title='Precursor charge', dpi=plot_dpi)
```
...
histogram(agg, 'Replicate RT SD (min)', dpi=plot_dpi,
