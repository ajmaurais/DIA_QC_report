...
from DIA_QC_report import filter_replicates
...
class TestFilterReplicates(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.parse_results = setup_functions.setup_multi_db(
            cls.data_dir, cls.work_dir, clear_dir=True, group_by_gene=False
        )
    ...
    def test_conflicting_rep_options_fail(self):
        ...
        command = ['--excludeRep', self.replicates[0], '--includeAll', self.db_path]
        result = setup_functions.run_main(
            filter_replicates._main, command, self.work_dir, prog=self.prog
        )
    ...
    def test_conflicting_project_options_fail(self):
        ...
        command = ['--excludeProject', self.projects[0], '--includeAll', self.db_path]
        result = setup_functions.run_main(
            filter_replicates._main, command, self.work_dir, prog=self.prog
        )
        self.assertEqual(result.returncode, 1)
    ...
    def test_include_all(self):
        ...
        command = ['--excludeProject', self.projects[0], self.db_path]
        result = setup_functions.run_main(
            filter_replicates._main, command, self.work_dir,
            prefix='test_include_all_exclude', prog=self.prog
        )
    ...
    def test_exclude_rep_option(self):
        ...
        for i in range(n_itterations):
            ...
            command = [f'-x={rep}' for rep in skip_reps]
            command.append(self.db_path)

            try:
                result = setup_functions.run_main(
                    filter_replicates._main, command, self.work_dir,
                    prefix=f'filter_replicates_{i + 1}', prog=self.prog
                )
                self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_exclude_project_option(self):
        ...
        for i, project in enumerate(projects):
            ...
            try:
                command = [f'-p={project}', self.db_path]
                result = setup_functions.run_main(
                    filter_replicates._main, command, self.work_dir,
                    prefix=f'filter_projects_{i + 1}', prog=self.prog
                )
                self.assertEqual(result.returncode, 0, result.stderr)
