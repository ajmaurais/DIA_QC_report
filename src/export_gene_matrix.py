...
def _main(argv, prog=None):
    ...
    for method in data:
        ...
        data[method] = data[method].rename(columns={'accession': 'Protein'})

        columns = list(data[method].columns)
        columns.pop(columns.index('gene_group'))
        columns.pop(columns.index('Protein'))
        metadata_index = columns.index('NCBIGeneID') if 'NCBIGeneID' in columns else len(columns)
        columns.insert(metadata_index, 'Protein')
